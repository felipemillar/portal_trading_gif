...
async def find_imacec():
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        imacec_matches = [s for s in series if "imacec" in (s.get("spanishTitle") or "").lower()]
        print(f"Total series Imacec encontradas: {len(imacec_matches)}")
        for s in imacec_matches[:10]:
            print(f"- ID: {s.get('seriesId')} | Titulo: {s.get('spanishTitle')} | Rango: {s.get('firstObservation')} a {s.get('lastObservation')}")

if __name__ == "__main__":
    asyncio.run(find_imacec())
